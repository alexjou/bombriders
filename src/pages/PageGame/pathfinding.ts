...
export function findPath(
  ...
  while (openList.length > 0) {
    ...
    for (const neighborCoord of neighborsCoords) {
      ...
      // Blocos sólidos são sempre impassáveis
      if (cellType === CellType.SOLID_BLOCK) {
        continue;
      }

      // Blocos destrutíveis são impassáveis, mas podemos considerá-los como rotas alternativas
      // com um custo muito alto (para quando não há outro caminho disponível)
      if (cellType === CellType.DESTRUCTIBLE_BLOCK) {
        // Ignora para pathfinding básico - deixamos essa opção comentada
        // para possível implementação de IA que pode destruir blocos
        continue;
      }
